...
## Ending Phase 2: the hand-off file

Once Phase 2 ends, also write out everything decided so far as one small, literal
file — not prose, not code — so a later step can pick it up without anyone re-typing
what you already worked out. This changes nothing about scope: the file still holds
no code and no prompt, only the facts Phase 1 and Phase 2 already fixed, plus a clear
blank everywhere a decision still needs a person who knows things you don't (which
real, registered source or sink fits; whether a drafted implementation actually works).

Write it as a Python file with exactly this shape — `OFFICE_NAME`, `AGENTS`,
`CONNECTIONS` — and nothing else:

- `OFFICE_NAME` — a short name for the office.
- `AGENTS` — a list, one entry per agent, each a `dict` with `name`, `kind`
  (`"source"`, `"sink"`, `"transform"`, or `"coordinator"`), `in_ports`, `out_ports`
  (lists, in the order you gave them), then:
  - a **registered agent** (a coordinator, or `record`) — always known, never blank:
    `registered_as` (its registered name — `"merge_synch"`, `"select"`, `"gate"`, or
    `"record"`) and `registered_args` (a dict — empty unless Phase 1/2 fixed something
    specific, e.g. what a `record` starts holding).
  - a **source or sink** — add `description` (Phase 2's own words) and
    `registered_as=None, registered_args={}` — always blank; you do not know
    DisSysLab's registered library, so never guess one.
  - a **transform** — add `description` (Phase 2's own words) and
    `body_kind=None, body_fn=None, body_prompt=None, approved=False` — always
    blank; Phase 2 deliberately stops at the English description, so there is
    never a real body here to write in.
- `CONNECTIONS` — a list of the same 4-tuples Phase 1 already produced:
  `(sender, sender's outbox, receiver, receiver's inbox)`.

Tell Pat this file exists and that it is for whoever builds the running version, not
something she needs to read herself, and hand it to her as one plain file to pass
along.
...
AGENTS = [
    dict(name="QUESTION", kind="source", in_ports=[], out_ports=["out"],
         description="The office's list of debate questions; emit one question at a time.",
    ...
    dict(name="DEBATER1", kind="transform", in_ports=["in"], out_ports=["out"],
         description="Given the question and the arguments made so far, write this debater's next argument: a short paragraph making and defending its position and answering the other debaters.",
         body_kind=None, body_fn=None, body_prompt=None, approved=False),
    dict(name="DEBATER2", kind="transform", in_ports=["in"], out_ports=["out"],
         description="Same as DEBATER1, arguing a second, differing stance.",
         body_kind=None, body_fn=None, body_prompt=None, approved=False),
    dict(name="DEBATER3", kind="transform", in_ports=["in"], out_ports=["out"],
         description="Same as DEBATER1, arguing a third, differing stance.",
         body_kind=None, body_fn=None, body_prompt=None, approved=False),
    dict(name="JOIN", kind="coordinator", in_ports=["in1", "in2", "in3"], out_ports=["out"],
         registered_as="merge_synch", registered_args={}),
    dict(name="MODERATOR", kind="transform", in_ports=["in"], out_ports=["continue", "finish"],
         description="Given the three latest arguments, judge whether the debate has reached a clear conclusion. If not, send the collected arguments back on continue for another round. If it has, write a final answer summing up the resolution and send it on finish.",
         body_kind=None, body_fn=None, body_prompt=None, approved=False),
    dict(name="TRANSCRIPT", kind="sink", in_ports=["in"], out_ports=[],
         description="Append each round's three arguments to a transcript file.",
         registered_as=None, registered_args={}),
    dict(name="ANSWER", kind="sink", in_ports=["in"], out_ports=[],
         description="Write the final answer to a file.",
         registered_as=None, registered_args={}),
]
...
Every `None` and every `approved=False` above is a real blank — GATE and JOIN are the
only two agents with nothing left to decide, because they are registered.
