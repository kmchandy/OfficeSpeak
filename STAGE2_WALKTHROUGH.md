# OfficeSpeak — Stage 2, another worked example (the fractions tutor)

*This is a second, complete worked example of Stage 2 — actually running an
office, not just describing one. `README.md`'s own Stage 2 section walks the
investment-club office through to a real `dsl run`; this page shows a
different office (a fractions tutor for a student) run a different way: live,
inside the Claude desktop app's Cowork mode, with a real language-model
grader. Read this if you want to see another domain, or the Cowork-based
workflow specifically, rather than the terminal-based one in `README.md`.*
