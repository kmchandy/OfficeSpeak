...
class _Parser:
    ...
    def _parse_for_each(self):
        ...
        while i < len(body):
            ...
            if _RE_IF.match(line.text) or _RE_SEND_TO.match(line.text):
                break
            sm = _RE_STEP.match(line.text)
            if not sm:
                raise ParseError(
                    f"expected step line '<id>: <verb> <object> -> [reads X,] enriches Y' "
                    f"or 'if' / 'send to', got {line.text!r}",
                    line=line.num,
                )
            last_vertex_id = self._add_step(sm, line, first_vertex_sources, last_vertex_id)
            i += 1

        if last_vertex_id is None:
            raise ParseError(
                "for-each body must contain at least one step line", line=header.num
            )

        # ---- optional if/elif/else block ----
        branches = []          # list of (outport_name, [(target, params, line_num), ...])
        else_outport_name = "else"
        if i < len(body):
            first_in_block = body[i]
            if first_in_block.indent == body_base and _RE_IF.match(first_in_block.text):
                branches, else_outport_name, i = self._parse_branches(
                    body, i, body_base
                )
                # Annotate router vertex with outports
                outport_order = [name for name, _ in branches]
                if "else" not in outport_order and else_outport_name not in outport_order:
                    pass  # caller decided no explicit else; we'll add implicit below
                for v in self.vertices:
                    if v["id"] == last_vertex_id:
                        v["_outports_explicit"] = outport_order[:]
                        v["_else_outport_name"] = else_outport_name
                        break

        # ---- unconditional send-tos ----
        unconditional = []
        while i < len(body):
            line = body[i]
            if line.indent != body_base:
                raise ParseError(
                    f"unexpected indent in for-each body (expected {body_base}, "
                    f"got {line.indent})",
                    line=line.num,
                )
            ...
            if not stm:
                raise ParseError(
                    f"expected 'send to <target>' (top of for-each body), got {line.text!r}",
                    line=line.num,
                )
            target = stm.group(1)
            args_str = stm.group(2) or ""
            params = _parse_kwargs(args_str, line=line.num)
            unconditional.append((target, params, line.num))
            i += 1

        # ---- resolve targets and emit edges ----
        if branches:
            # Find router vertex to potentially add implicit-else outport
            router_v = None
            for v in self.vertices:
                if v["id"] == last_vertex_id:
                    router_v = v
                    break
            existing_outports = list(router_v["_outports_explicit"])
            if else_outport_name not in existing_outports:
                # Implicit else outport: catches everything not matched by any if/elif
                existing_outports.append(else_outport_name)
                # Implicit else branch has no explicit send-tos; only unconditional sends apply
                branches.append((else_outport_name, []))
            router_v["outports"] = existing_outports

            for outport, sends in branches:
                for target, params, line_num in sends:
                    dst, port = self._resolve_target(target, params, line_num)
                    self.edges.append(
                        {"from": [last_vertex_id, outport], "to": [dst, port]}
                    )
            # Apply unconditional to every branch
            for target, params, line_num in unconditional:
                dst, port = self._resolve_target(target, params, line_num)
            ...
            if not unconditional:
                raise ParseError(
                    "for-each body has no 'send to' lines", line=header.num
                )
            for target, params, line_num in unconditional:
                dst, port = self._resolve_target(target, params, line_num)
                self.edges.append(
                    {"from": [last_vertex_id, "out"], "to": [dst, port]}
                )
    ...
    def _add_step(
        ...
        sm: re.Match,
        line: _Line,
        first_vertex_sources: list[str],
        last_vertex_id: Optional[str],
    ) -> str:
        step_id = sm.group(1)
        ...
        # Implicit edge: from previous vertex (or sources) to this one
        if last_vertex_id is None:
            for src_id in first_vertex_sources:
                self.edges.append({"from": [src_id, "out"], "to": [vid, "in_"]})
        else:
            self.edges.append({"from": [last_vertex_id, "out"], "to": [vid, "in_"]})
